...
# WMS parameters
wms_server = "http://geoserver.dataeng.io:443/geoserver/loceng/wms"
wms_layers = ["locality_bdys_display"]
wms_styles = ""
wms_format = "image/png"
wms_version = "1.3.0"  # Only v1.1.1 and v1.3.0 tested
wms_srid = "EPSG:900913"  # web mercator

# WFS parameters
wfs_server = "http://geoserver.dataeng.io:443/geoserver/loceng/ows"
wfs_layers = ["locality_bdys_display"]
wfs_format = "application/json"  # aka GeoJSON
wfs_version = "1.0.0"  # Only v1.0.0 tested
wfs_srid = "EPSG:4283"  # GDA94 lat/long
...
def create_requests():
    ...
    for bounds in bounds_list:
        # Construct URL
        if request_type == "WMS":
            url_params = {"LAYERS": ",".join(wms_layers), "FORMAT": wms_format, "SERVICE": request_type,
                          "VERSION": wms_version, "REQUEST": "GetMap", "STYLES": wms_styles,
                          "SRS": wms_srid, "WIDTH": str(map_image_width), "HEIGHT": str(map_image_height),
                          "BBOX": ",".join(bounds)}
            url = "".join([wms_server, "?", urllib.urlencode(url_params)])
        else:
            url_params = {"TYPENAMES": ",".join(wfs_layers), "OUTPUTFORMAT": wfs_format, "SERVICE": request_type,
                          "VERSION": wfs_version, "REQUEST": "GetFeature", "SRSNAME": wfs_srid, "BBOX": ",".join(bounds)}
            url = "".join([wfs_server, "?", urllib.urlencode(url_params)])
