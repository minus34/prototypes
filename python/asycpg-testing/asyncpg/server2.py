...
async def get_data(request, ml, mb, mr, mt, z):
    ...
    async with engine.acquire() as connection:
        ...
        sql = "SELECT json_build_object('type', 'Feature','geometry', ST_AsGeoJSON(geom, 5)::json, 'properties', " \
              "json_build_object('id', x::text || y::text, 'percent', percent, 'difference', difference)) AS geojson " \
              "FROM {1}.{2} " \
